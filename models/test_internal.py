...
from models.uamnet import CUSTOM_OBJECTS, set_mc_dropout
...
def mc_predict(model, X, yr_max, n_mc, batch, logvar_clip=None):
    ...
    # Deterministic classification: BatchNorm and all Dropout layers are
    # in inference mode. This matches the final notebook protocol.
    set_mc_dropout(model, False)
    cls_batches = []
    for start in range(0, len(X), batch):
        stop = start + batch
        cls, _ = model(X[start:stop], training=False)
        cls_batches.append(cls.numpy())
    pred_cls = np.concatenate(cls_batches, axis=0).argmax(axis=1)

    # MC sampling is restricted to the custom dropout layer in the
    # regression branch; BatchNorm remains in inference mode.
    set_mc_dropout(model, True)
    for _ in range(n_mc):
        mu_batches, var_batches = [], []
        for start in range(0, len(X), batch):
            stop = start + batch
            # Keep BatchNorm and ordinary Dropout in inference mode.
            _, reg = model(X[start:stop], training=False)
...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Clip log-variance to [-4, 2] (default).",
    )
    parser.add_argument(
        "--no-clip-logvar",
        dest="clip_logvar",
        action="store_false",
        help="Disable log-variance clipping for diagnostic comparisons.",
    )
