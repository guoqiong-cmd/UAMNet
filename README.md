...
```bash
python models/train_uamnet.py \
  --data data/uamnet_split_data.npz \
  --out weights/retrained \
  --seed 42
```

The training script performs a mean-only MSE warm-up on the predicted aging
mean `mu`, followed by NLL fine-tuning of the mean and log-variance outputs.

The bundled `weights/UAMNet_best.h5` is the corrected model from seed 20.
The manuscript summary statistics were obtained from three fixed-split runs
with seeds 20, 21, and 22; rerunning with another seed, including seed 42,
is expected to give a nearby but not identical result.

## Internal Test Evaluation

```bash
python models/test_internal.py \
  --data data/uamnet_split_data.npz \
...
Log-variance clipping to `[-4, 2]` is enabled by default to match the
manuscript protocol. Use `--no-clip-logvar` only for diagnostic comparisons.
